...
export async function checkConnectors(): Promise<ConnectorHealth[]> {
  ...
  let banking: ConnectorHealth = process.env.OPEN_BANKING_API_URL && process.env.OPEN_BANKING_ACCESS_TOKEN
  ...
  const zatca: ConnectorHealth = process.env.ZATCA_API_URL && process.env.ZATCA_CSID && process.env.ZATCA_CSID_SECRET
...
const bankTransactionSchema = z.object({ id: z.string(), accountId: z.string(), bookedAt: z.string().datetime(),
  description: z.string(), amount: z.number(), currency: z.string().length(3), direction: z.enum(['credit', 'debit']) });
export async function fetchOpenBankingTransactions(since: string) {
...
export async function submitZatcaDocument(input: { mode: 'clearance' | 'reporting'; invoiceHash: string; uuid: string; invoiceBase64: string }) {
