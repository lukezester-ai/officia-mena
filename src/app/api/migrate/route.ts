...
export async function POST(request: Request) {
  ...
  try {
    const connectionString =
      process.env.MIGRATION_DATABASE_URL || process.env.POSTGRES_URL || process.env.STORAGE_DATABASE_URL;
    if (!connectionString) {
      return NextResponse.json(
        { error: 'MIGRATION_DATABASE_URL, POSTGRES_URL, or STORAGE_DATABASE_URL is not set' },
        { status: 503 }
      );
    }
